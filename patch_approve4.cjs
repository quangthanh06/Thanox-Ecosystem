...
const matchRegex = /  const approveTopup = \(id: string\) => \{[\s\S]*?syncTransactionToSupabase\(newTx\);\n\s*showToast\([\s\S]*?\);\n  \};/m;
...
const newFunc = `  const approveTopup = async (id: string) => {
    ...
    try {
      ...
      showToast(\`Ðã duy?t n?p ti?n an toàn qua Server!\`, 'success');
    } catch (e) {
      console.error(e);
...
if(matchRegex.test(file)) {
  ...
  console.log('Patched approveTopup perfectly with generic regex!');
} else {
  console.log('Still could not find match using regex!');
}
