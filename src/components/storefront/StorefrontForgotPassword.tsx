        ...
        <div className="lg:col-span-7 xl:col-span-6 flex flex-col justify-center">
          <div className="glass-prominent border border-white/12 shadow-[0_30px_70px_rgba(0,0,0,0.85)] rounded-3xl p-6 sm:p-10 space-y-6">
            ...
            <div className="flex items-center justify-between border-b border-white/8 pb-4">
              <Link
                to="/login"
                className="inline-flex items-center gap-2 text-xs font-semibold text-[#938EB5] hover:text-white transition-colors"
              >
                <ArrowLeft className="w-4 h-4" />
                <span>Quay lại Đăng nhập</span>
              </Link>
              <span className="text-xs text-[#C084FC] font-mono font-bold">Bảo mật 2 lớp</span>
            </div>

            {/* Header / Icon */}
            <div className="space-y-2">
              <div className="w-10 h-10 rounded-2xl bg-[#7C3AED]/20 border border-[#7C3AED]/30 flex items-center justify-center text-[#C084FC] shadow-lg shadow-[#7C3AED]/20 mb-2">
              ...
              {/* SPECIAL ANIMATED COLOR MOTION BANNER */}
              <div className="p-4 rounded-2xl glass-subtle border border-[#06B6D4]/40 shadow-lg shadow-cyan-500/10 space-y-2">
              ...
              <p className="text-xs sm:text-sm text-[#938EB5] pt-1">
                {step === 'request'
            ...
            {successMessage && (
              <div className="p-3.5 rounded-2xl bg-emerald-500/12 border border-emerald-500/25 text-emerald-300 text-xs flex items-center gap-2.5 shadow-sm">
            ...
            {step === 'request' ? (
              ...
              <form onSubmit={handleResetSubmit} className="space-y-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-[#F4F2FF]">Mã xác thực (OTP)</label>
                  <input
                    type="text"
                    value={resetCode}
                    onChange={(e) => setResetCode(e.target.value)}
                    placeholder="Nhập mã xác thực (VD: 889922)"
                    className="w-full glass-input rounded-2xl px-4 py-3 text-xs sm:text-sm text-[#F4F2FF] font-mono"
                    required
                  />
                </div>

                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-[#F4F2FF]">Mật khẩu mới (tối thiểu 6 ký tự)</label>
                  <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-[#938EB5]">
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-[#F4F2FF]">Xác nhận mật khẩu mới</label>
                  <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-[#938EB5]">
                      <Lock className="w-4 h-4" />
                    </div>
            ...
            {/* Support from Admin */}
            <div className="p-4 rounded-2xl glass-subtle border border-white/6 space-y-2 text-xs">
              ...
              <div className="grid grid-cols-2 gap-2 pt-1">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="p-2 rounded-xl bg-blue-500/10 border border-blue-500/20 text-blue-300 hover:bg-blue-500/20 text-center font-bold text-[11px] transition-colors"
                >
                  Telegram: @quangthank
                </a>
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="p-2 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 hover:bg-emerald-500/20 text-center font-bold text-[11px] transition-colors"
                >
                  Zalo: 0889696810
                </a>
              </div>
            ...
            <div className="pt-2 text-center">
              <Link
                to="/login"
                className="text-xs text-[#938EB5] hover:text-white transition-colors"
              >
                Đã nhớ lại mật khẩu? <span className="text-[#C084FC] font-bold">Đăng nhập ngay</span>
              </Link>
            </div>
          </div>
        </div>
