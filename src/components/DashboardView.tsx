...
export const DashboardView: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 glass-prominent border border-white/10 rounded-3xl p-4 sm:p-6 shadow-xl">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <h2 className="font-display text-lg sm:text-xl font-black text-[#F4F2FF] tracking-tight">
              Chào mừng trở lại, Admin
            </h2>
            <span className="text-xs px-2.5 py-0.5 rounded-full bg-[#7C3AED]/25 text-[#C084FC] font-bold border border-purple-500/30">
              Live
            </span>
          </div>
        ...
        <div className="flex items-center gap-2.5 shrink-0">
          <Button
            ...
            leftIcon={<Zap className="w-3.5 h-3.5 text-[#22D3EE]" />}
          >
            Cửa Hàng Khách
          </Button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
        ...
        <Card className="lg:col-span-8 p-5 space-y-4" variant="default">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-white/6">
            <div>
              <h3 className="font-display font-black text-sm text-[#F4F2FF]">Biểu Đồ Doanh Thu</h3>
              <p className="text-xs text-[#938EB5]">Biến động doanh thu theo thời gian thực</p>
            </div>

            <div className="flex items-center gap-1 glass-subtle p-1 rounded-2xl border border-white/6 text-xs">
          ...
          <div className="relative pt-2">
            <div className="h-60 w-full flex items-end relative overflow-hidden">
                      ...
                      {chartData.map((d, i) => {
                        ...
                        return (
                          <circle
                            ...
                            cy={cy}
                            r="4"
                            className="fill-[#0B0B14] stroke-[#C084FC] stroke-2 hover:r-6 transition-all cursor-pointer"
              ...
              {hoveredPoint && (
                <div
                  className="absolute glass-prominent border border-white/20 px-3 py-1.5 rounded-2xl shadow-2xl text-xs pointer-events-none z-20 transform -translate-x-1/2 -translate-y-full"
                  style={{
                    left: `${(hoveredPoint.x / (chartDays * 50)) * 100}%`,
                    top: `${(hoveredPoint.y / 200) * 100}%`,
                  }}
                >
                  <div className="text-[10px] text-[#938EB5]">{hoveredPoint.date}</div>
            ...
            {/* X-axis labels */}
            <div className="flex justify-between pt-3 border-t border-white/6 text-[10.5px] text-[#938EB5]">
              {chartData.map((d, i) => (
        ...
        <Card className="lg:col-span-4 p-5 space-y-4" variant="default">
          <div className="pb-3 border-b border-white/6">
            <h3 className="font-display font-black text-sm text-[#F4F2FF]">Tỷ Trọng Danh Mục</h3>
            <p className="text-xs text-[#938EB5]">Phân bổ doanh thu theo nhóm sản phẩm</p>
          </div>

          {categoryEntries.length === 0 ? (
            <div className="py-12 text-center text-xs text-[#938EB5]">Chưa có dữ liệu phân loại</div>
          ) : (
                ...
                return (
                  <div key={cat} className="space-y-1.5">
                    <div className="flex justify-between text-xs font-bold">
                      <span className="text-[#F4F2FF]">{cat}</span>
                      <span className="text-emerald-300">{percent}% ({amount.toLocaleString('vi-VN')}đ)</span>
                    </div>
          ...
          {orders.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs whitespace-nowrap min-w-[500px]">
                <thead>
                  <tr className="text-[#938EB5] border-b border-white/6 uppercase text-[10px] tracking-wider">
                    <th className="pb-2.5 font-bold">Mã đơn</th>
                    <th className="pb-2.5 font-bold">Khách hàng</th>
                    <th className="pb-2.5 font-bold">Sản phẩm</th>
                    <th className="pb-2.5 font-bold">Tổng tiền</th>
                    <th className="pb-2.5 font-bold text-right">Trạng thái</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/6">
                  {orders.slice(0, 5).map((o) => (
                    <tr key={o.id} className="hover:bg-white/[0.02] transition-colors">
                      <td className="py-3 font-mono font-bold text-[#F4F2FF]">{o.orderCode}</td>
                      <td className="py-3 text-[#F4F2FF] font-medium">{o.userName}</td>
                      <td className="py-3 text-[#938EB5] max-w-[160px] truncate">{o.productName}</td>
          ...
          {topProducts.length === 0 ? (
            ...
            <div className="space-y-3">
              {topProducts.map((p, idx) => (
                <div
                  key={p.id}
                  className="flex items-center justify-between p-3 rounded-2xl glass-subtle border border-white/6 hover:border-[#7C3AED]/30 transition-all"
                >
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="font-bold text-xs text-[#F4F2FF] truncate">{p.name}</div>
                      <div className="text-[10.5px] text-[#938EB5]">{p.category}</div>
                    </div>
                  </div>
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <div className="text-[10.5px] text-[#938EB5]">Đã bán {p.soldCount}</div>
                  </div>
                </div>
              ))}
            </div>
