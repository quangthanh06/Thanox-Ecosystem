...
function replaceFunctionSmart(fileContent, funcName, newFuncStr) {
  ...
  // Find "=> {" or ") {" or "() => {" after startIndex
  // Because it could be `async (...) => {` or `() => {`
...
file = replaceFunctionSmart(file, 'register', newRegister);
file = replaceFunctionSmart(file, 'logout', newLogout);
